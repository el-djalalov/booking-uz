...
import { AuthResponse, ApiResponse } from "./api";
...
export const getAuthToken = async (): Promise<string> => {
  ...
  authPromise = (async () => {
    console.log("Performing server-side authentication...");
    try {
      ...
      if (response.data.success && response.data.data?.auth_token) {
        console.log("Server-side authentication successful. Caching token.");
      ...
      if (axios.isAxiosError(error)) {
        ...
        console.error("Unknown error during server-side authentication:", error);
      }
